        ...
        if "levels" in data:
            for level_key, level_data in data["levels"].items():
                if isinstance(level_data, dict):
                    # Extract level name
                    level_name = level_data.get(
                        "level_name", level_data.get("level", level_key)
                    )
                        ...
                        for story in stories:
                            if isinstance(story, dict):
                                story_title = story.get(
                                    "title", story.get("story", "Unknown Story")
                                )
...
def merge_all_data():
    ...
    sources = [
        ...
        ("data/pdf_questions_all.json", "PDF Files (9 levels)"),
    ]
    ...
    for q in unique_questions:
        ...
        story_value = q.get("story", q.get("story_title", "")).strip()
        level_value = str(level).upper()

        # Fix: If story equals level or "Level {level}", set to "Unknown Story"
        # This prevents "Level X" from being used as story name
        if story_value == level_value or story_value == f"Level {level_value}":
            story_value = "Unknown Story"
    ...
    for level in all_levels:
        ...
        for story_name, story_questions in sorted(stories.items()):
            ...
            for i, q in enumerate(story_questions):
                q_copy = q.copy()
                q_copy["id"] = (
                    f"{level.lower()}-{story_name[:20].replace(' ', '-')}-{i:03d}"
                )
